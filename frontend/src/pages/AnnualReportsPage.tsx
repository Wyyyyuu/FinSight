          ...
          <div className="min-w-0 space-y-5">
            <Card className="p-5 sm:p-6">
              ...
              <form onSubmit={(event) => void analyze(event)}>
                ...
                <div className="mt-3 flex flex-wrap items-center justify-between gap-3"><p className="text-xs text-fin-text-secondary">回答附带来源页码，证据不足时会明确说明。</p>{analyzing ? <Button key="cancel" type="button" onClick={(event) => { event.preventDefault(); analyzeController.current?.abort(); setAnalyzing(false); setAnalysisError('已停止等待结果；后台可能仍在建立索引，可稍后重新查询。'); }}><X size={16} />停止等待</Button> : <Button key="analyze" type="submit" variant="primary" disabled={!question.trim() || !selectedDocuments.length || uploading || demoLoading}><Search size={16} />分析所选资料<ArrowRight size={15} /></Button>}</div>
              </form>
            ...
            {analyzing && <Card className="p-7" role="status" aria-live="polite"><div className="flex items-center gap-3"><Loader2 size={20} className="animate-spin text-fin-primary motion-reduce:animate-none" /><div><p className="font-medium">正在检索并核对年报证据</p><p className="mt-1 text-sm text-fin-text-secondary">首次处理长年报可能需数分钟，后续查询会复用索引。</p><p className="mt-1 text-sm text-fin-text-secondary">完成后将展示回答、引用原文和实际执行步骤。</p></div></div></Card>}
